...
def predict():
     if request.method == 'POST':
            ...
            Age = int(request.form['ageBox'])
                
            NatoKBox = float(request.form['naToKBox'])
            
            print([[Age, Male, Cholesterol_Status, NatoKBox, BP_HIGH, BP_LOW]])

            prediction = model.predict([[Age, Male, Cholesterol_Status, NatoKBox, BP_HIGH, BP_LOW]])

            return render_template('Drug_App_Template.html', prediction_text= prediction[0] )
